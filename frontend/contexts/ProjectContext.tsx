...
function getUniqueImportedProjectName(name: string, projects: Project[]): string {
  const baseName = name.trim() || 'Imported Project'
  ...
  let index = 2
  while (existingNames.has(`${baseName} (imported ${index})`.toLowerCase())) {
    index += 1
  }

  return `${baseName} (imported ${index})`
}
...
export function ProjectProvider({ children }: { children: React.ReactNode }) {
  ...
  const importProject = useCallback((project: Project): Project => {
    const importedProject = recoverAssetUrls(migrateProject(project))
    const uniqueName = getUniqueImportedProjectName(importedProject.name, projects)
