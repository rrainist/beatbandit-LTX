...
async function testCompleteImport(): Promise<void> {
  ...
  assert.equal(result.project.timelines[0].clips.length, 3)
  assert.equal(result.project.timelines[0].clips[1].assetId, 'asset_video_002')
  assert.equal(result.project.timelines[0].clips[1].type, 'video')
  assert.equal(result.project.timelines[0].clips[2].type, 'audio')
  assert.deepEqual(result.project.timelines[0].clips[1].linkedClipIds, [result.project.timelines[0].clips[2].id])
