...
import { BeatBanditBatchGenerateModal, type BeatBanditBatchScope } from '../components/BeatBanditBatchGenerateModal'
...
const BEATBANDIT_BATCH_CANCEL_ERROR = 'BeatBandit batch generation cancelled'

interface BeatBanditBatchJob {
  jobKey: string
  label: string
  sourceAsset: Asset
  prompt: string
...
interface BeatBanditBatchFailure {
  jobKey: string
  label: string
  error: string
}
...
export function VideoEditor() {
  ...
  const [beatBanditBatchCurrentLabel, setBeatBanditBatchCurrentLabel] = useState<string | null>(null)
  const [beatBanditBatchFailures, setBeatBanditBatchFailures] = useState<BeatBanditBatchFailure[]>([])
  const [beatBanditBatchLastJobs, setBeatBanditBatchLastJobs] = useState<BeatBanditBatchJob[]>([])
  const beatBanditBatchCancelRequestedRef = useRef(false)
  const beatBanditBatchResolverRef = useRef<{
    resolve: (value: { videoUrl: string; videoPath: string }) => void
  ...
  const getBatchClipAsset = useCallback((clip: TimelineClip) => {
    const assetId = clip.assetId || clip.asset?.id
    if (assetId) {
      return assets.find(a => a.id === assetId) || (clip.asset as Asset | undefined) || null
    }
  ...
  const getBeatBanditBatchJobFromClip = useCallback((clip: TimelineClip): BeatBanditBatchJob | null => {
    ...
    if (!params || params.mode === 'text-to-image' || params.mode === 'audio-to-video' || params.mode === 'retake') {
      return null
    }
    ...
    const settings = shouldVideoGenerateWithLtxApi
      ...
      : rawSettings
    const beatbanditShotId = asset.importMeta?.beatbanditShotId
    const beatbanditVariantKey = clip.beatbanditVariantKey
    const laneSuffix = clip.beatbanditLaneIndex ? ` (Lane ${clip.beatbanditLaneIndex})` : ''

    return {
      jobKey: beatbanditVariantKey || beatbanditShotId || asset.id,
      label: `${(asset.prompt || clip.importedName || `BeatBandit Shot ${beatbanditShotId || asset.id}`).slice(0, 84)}${laneSuffix}`,
      sourceAsset: asset,
      prompt,
      imagePath,
      settings,
  ...
  const buildBeatBanditBatchJobs = useCallback((scope: BeatBanditBatchScope) => {
    ...
    const sourceTimelines = scope === 'timeline'
      ...
      : timelines.map(timeline => timeline.id === activeTimeline?.id ? { ...timeline, clips } : timeline)

    const jobs: BeatBanditBatchJob[] = []
    ...
    return jobs
  }, [activeTimeline, clips, timelines, getBeatBanditBatchJobFromClip])

  const beatBanditEligibleCounts = useMemo(() => ({
    timeline: buildBeatBanditBatchJobs('timeline').length,
  ...
  useEffect(() => {
    ...
    if (beatBanditBatchCancelRequestedRef.current) {
      beatBanditBatchResolverRef.current = null
      pending.reject(new Error(BEATBANDIT_BATCH_CANCEL_ERROR))
      return
    }

    if (regenError) {
      beatBanditBatchResolverRef.current = null
      pending.reject(new Error(regenError))
      return
    }

    if (regenVideoUrl && regenVideoPath) {
      beatBanditBatchResolverRef.current = null
      pending.resolve({ videoUrl: regenVideoUrl, videoPath: regenVideoPath })
    }
  }, [isRegenerating, regenError, regenVideoUrl, regenVideoPath])

  const runBeatBanditBatchVideoGeneration = useCallback(async (job: BeatBanditBatchJob) => {
    if (beatBanditBatchCancelRequestedRef.current) {
      throw new Error(BEATBANDIT_BATCH_CANCEL_ERROR)
    }

    regenReset()

    return new Promise<{ videoUrl: string; videoPath: string }>((resolve, reject) => {
      beatBanditBatchResolverRef.current = { resolve, reject }
      regenGenerate(job.prompt, job.imagePath, job.settings).catch((error) => {
        if (beatBanditBatchResolverRef.current) {
          beatBanditBatchResolverRef.current = null
        }
        reject(error)
      })
    })
  }, [regenGenerate, regenReset])

  const replaceBeatBanditShotClips = useCallback((job: BeatBanditBatchJob, generatedAsset: Asset) => {
    if (!currentProjectId) return

    const sourceAsset = job.sourceAsset
    const shouldReplaceClip = (clip: TimelineClip) => {
      if (job.beatbanditVariantKey) {
        return clip.beatbanditVariantKey === job.beatbanditVariantKey
      }

      const clipAsset = getBatchClipAsset(clip)
      if (!clipAsset) return false
      if (clipAsset.id === sourceAsset.id) return true
      if (!job.beatbanditShotId) return false
      return clipAsset.importMeta?.source === 'beatbandit'
        && clipAsset.importMeta?.beatbanditShotId === job.beatbanditShotId
        && clipAsset.type === 'image'
    }
    const toGeneratedClip = (clip: TimelineClip): TimelineClip => ({
      ...clip,
      assetId: generatedAsset.id,
      asset: generatedAsset,
      type: 'video',
  ...
  const runBeatBanditBatchJobs = useCallback(async (jobs: BeatBanditBatchJob[]) => {
    ...
    setBeatBanditBatchLastJobs(jobs)
    setBeatBanditBatchTotal(jobs.length)
    setBeatBanditBatchCompleted(0)
    setBeatBanditBatchFailures([])
    setBeatBanditBatchCurrentLabel(null)
    beatBanditBatchCancelRequestedRef.current = false

    if (jobs.length === 0) {
      setBeatBanditBatchStatus('done')
      return
    }

    setBeatBanditBatchStatus('running')

    for (const job of jobs) {
      if (beatBanditBatchCancelRequestedRef.current) break

      setBeatBanditBatchCurrentLabel(job.label)

      try {
        const { videoUrl, videoPath } = await runBeatBanditBatchVideoGeneration(job)
        const { path: finalPath, url: finalUrl } = await copyToAssetFolder(videoPath, videoUrl, currentProject?.assetSavePath)
        const sourceParams = job.sourceAsset.generationParams
        const generatedAsset = addAsset(currentProjectId, {
          type: 'video',
          path: finalPath,
          url: finalUrl,
          prompt: job.prompt,
          resolution: job.settings.videoResolution,
          duration: job.settings.duration,
          thumbnail: finalUrl,
          bin: job.sourceAsset.bin,
          colorLabel: job.sourceAsset.colorLabel,
          importMeta: job.sourceAsset.importMeta,
        ...
        replaceBeatBanditShotClips(job, generatedAsset)
        setBeatBanditBatchCompleted(prev => prev + 1)
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        if (message === BEATBANDIT_BATCH_CANCEL_ERROR) break
        logger.error(`BeatBandit batch generation failed: ${message}`)
        setBeatBanditBatchFailures(prev => [...prev, {
          jobKey: job.jobKey,
          label: job.label,
          error: message,
        }])
    ...
    setBeatBanditBatchCurrentLabel(null)
    setBeatBanditBatchStatus(beatBanditBatchCancelRequestedRef.current ? 'cancelled' : 'done')
    beatBanditBatchCancelRequestedRef.current = false
  }, [currentProject?.assetSavePath, currentProjectId, addAsset, replaceBeatBanditShotClips, regenReset, runBeatBanditBatchVideoGeneration])

  const openBeatBanditBatchGenerateModal = useCallback((scope: BeatBanditBatchScope) => {
    setBeatBanditBatchScope(scope)
    setShowBeatBanditBatchModal(true)
    if (beatBanditBatchStatus !== 'running') {
      setBeatBanditBatchStatus('idle')
      setBeatBanditBatchTotal(0)
      setBeatBanditBatchCompleted(0)
      setBeatBanditBatchCurrentLabel(null)
      setBeatBanditBatchFailures([])
    }
  ...
  const handleRetryFailedBeatBanditBatchGeneration = useCallback(() => {
    ...
    const jobs = beatBanditBatchLastJobs.filter(job => failedKeys.has(job.jobKey))
    void runBeatBanditBatchJobs(jobs)
  }, [beatBanditBatchFailures, beatBanditBatchLastJobs, runBeatBanditBatchJobs])

  const handleCancelBeatBanditBatchGeneration = useCallback(() => {
    beatBanditBatchCancelRequestedRef.current = true
    regenCancel()
  }, [regenCancel])
  ...
  const menuDefinitions: MenuDefinition[] = useMemo(() => buildMenuDefinitions({
    ...
    setShowImportTimelineModal, setShowExportModal,
    openBeatBanditBatchGenerateModal: () => openBeatBanditBatchGenerateModal('project'),
    beatBanditProjectMissingShotCount: beatBanditEligibleCounts.project,
    batchGenerationBusy: beatBanditBatchStatus === 'running' || isRegenerating,
  ...
  }), [selectedClip, selectedClipIds, clips, tracks, subtitles, snapEnabled, showEffectsBrowser, showSourceMonitor, showPropertiesPanel, _showICLoraPanel, sourceAsset, activeTool, activeTimeline, timelines, handleInsertEdit, handleOverwriteEdit, kbLayout, openBeatBanditBatchGenerateModal, beatBanditEligibleCounts.project, beatBanditBatchStatus, isRegenerating])
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        <div className="h-9 bg-zinc-900 border-t border-zinc-800 flex items-center px-3 gap-2 flex-shrink-0">
          ...
          <Button
            variant="outline"
            size="sm"
            className="h-6 border-blue-700/50 text-blue-300 text-[10px] px-2 hover:bg-blue-900/20"
            onClick={() => openBeatBanditBatchGenerateModal('timeline')}
            disabled={beatBanditEligibleCounts.timeline === 0 || beatBanditBatchStatus === 'running' || isRegenerating}
            title="Generate missing BeatBandit shots in the active timeline"
          >
      ...
      <BeatBanditBatchGenerateModal
        open={showBeatBanditBatchModal}
        onClose={() => {
          if (beatBanditBatchStatus !== 'running') {
            setShowBeatBanditBatchModal(false)
          }
        }}
        scope={beatBanditBatchScope}
        onScopeChange={setBeatBanditBatchScope}
        eligibleCounts={beatBanditEligibleCounts}
        status={beatBanditBatchStatus}
        total={beatBanditBatchTotal}
        completed={beatBanditBatchCompleted}
        failures={beatBanditBatchFailures}
        currentLabel={beatBanditBatchCurrentLabel}
        progress={regenProgress}
        statusMessage={regenStatusMessage}
