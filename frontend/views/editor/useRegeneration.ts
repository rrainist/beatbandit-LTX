...
export function useRegeneration(params: UseRegenerationParams) {
  ...
  useEffect(() => {
    if (regenVideoUrl && regenVideoPath && regeneratingAssetId && currentProjectId && !isRegenerating) {
      ;(async () => {
        ...
        const sourceAsset = assets.find(a => a.id === regeneratingAssetId) || null
        const sourceClip = regeneratingClipId ? clips.find(c => c.id === regeneratingClipId) || null : null

        // Video-on-video regeneration should create a new take.
        if (sourceAsset?.type === 'video') {
          ...
          // Image-source BeatBandit shots generate a brand new video asset.
          const sourceParams = sourceAsset?.generationParams
          const generatedDuration = sourceClip?.duration || sourceParams?.duration || sourceAsset?.duration || 5
          const generatedResolution = sourceParams?.resolution || sourceAsset?.resolution || ''
          const generatedPrompt = sourceParams?.prompt || sourceAsset?.prompt || ''
          const generatedAsset = addAsset(currentProjectId, {
            ...
            thumbnail: finalUrl,
            bin: sourceAsset?.bin,
            colorLabel: sourceAsset?.colorLabel,
            importMeta: sourceAsset?.importMeta,
  ...
  }, [regenVideoUrl, regenVideoPath, regeneratingAssetId, regeneratingClipId, currentProjectId, isRegenerating, assetSavePath, assets, clips, addAsset, addTakeToAsset, setClips, regenReset])
